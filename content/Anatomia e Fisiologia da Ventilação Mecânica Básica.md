...
# Anatomia e Fisiologia da Ventilação Mecânica Básica
...
## Identificação Visual do Modo Ventilatório
...
- **A análise visual das curvas de Fluxo e Pressão permite o reconhecimento exato do modo ventilatório de forma independente do que está escrito no painel da máquina.**
    ...
    - Matriz de Diferenciação: PCV vs. PSV (O Desafio Clínico):
        ...
        - Curva de Pressão: Em ambos os modos (PCV e PSV) a pressão é constante (onda quadrada com platô perfeito).
            
        - Curva de Fluxo: Em ambos, o fluxo é decrescente.
            
        - O Fator de Desempate (A Chave Diagnóstica): O Tempo Inspiratório (Ti) e a Ciclagem.
            
            - No PCV: O Tempo Inspiratório (Ti) é absolutamente FIXO e idêntico em todas as incursões. O término da inspiração ignora o esforço do paciente.
                
            - No PSV (A Tríade Visual): A ciclagem a fluxo é a assinatura visual definitiva. O Tempo Inspiratório (Ti) é VARIÁVEL, respeitando a demanda do paciente; a máquina cicla no mesmo percentual de queda do fluxo inspiratório (% pFluxo).
                

# Prescrição, Parâmetros e Monitorização (Settings)

## Estruturação da Prescrição e Metas de Monitorização

- **O setting ventilatório exige a configuração de variáveis independentes (fixadas pelo médico) e a vigilância estrita das variáveis dependentes (respostas fisiológicas), sempre balizadas pelo Peso Predito.**
    
    - Variáveis Universais (Presentes em todos os modos):
        
        - PEEP (Pressão Positiva Expiratória Final).
            
        - FiO2 (Fração Inspirada de Oxigênio).
            
    - Setting em VCV (Assisto-Controlado):
        
        - Parâmetros Ajustados: Volume Corrente (Vt), PEEP, FiO2, Frequência Respiratória (FR) e Relação I:E (Fluxo).
            
        - Cálculo do Vt Alvo: O volume deve ser estritamente calculado com base no Peso Predito (IBW - Ideal Body Weight), utilizando a régua de 4 a 8 ml/kg (idealmente em torno de 6 ml/kg para proteção pulmonar).
            
        - Monitorização de Alarme (Variáveis Dependentes): Pressão de Pico (Ppk), Pressão de Platô (Ppl), Volume Minuto (Vm) e SpO2.
            
    - Setting em PCV (Assisto-Controlado):
        
        - Parâmetros Ajustados: Delta de Pressão Inspiratória (ΔP ou Pinsp), Tempo Inspiratório (Ti fixo), PEEP, FiO2 e FR.
            
        - Monitorização de Alarme (Variáveis Dependentes): O Volume Corrente (Vt) resultante. Deve-se titular a Pressão Inspiratória para atingir o Vt protetor de ~6 a 8 ml/kg do Peso Predito. Vigiar Vm e SpO2.
            
    - Setting em PSV (Espontâneo):
        
        - Parâmetros Ajustados: Pressão de Suporte (ΔPsup), PEEP, FiO2 e Critério de Ciclagem (% do pico de fluxo, ex: 25%).
            
        - Monitorização de Alarme: Frequência Respiratória do paciente, Vt gerado, Volume Minuto (Vm) e SpO2.
            
    - Ferramenta Prática Beira-Leito (Pérola de Otimização):
        
        - Assim que o paciente for intubado, calcule e fixe no leito uma tabela com o Peso Predito (baseado na altura e sexo) e os equivalentes volumétricos exatos para 4, 5, 6, 7 e 8 ml/kg.
            
        - Exemplo prático: Homem, Altura 1,71m, Peso Predito 67kg → Vt protetor de 6ml/kg = 402 ml. Ajustar os modos VCV ou PCV para atingir rigorosamente esta meta.
